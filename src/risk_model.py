...
def run():
 raw=load_and_validate(ROOT/"data/patient_records.csv"); print("Missing values before splitting:\n",raw.isna().sum()); tr,te=train_test_split(raw,test_size=.2,random_state=42,stratify=raw[TARGET_COL]); tr=clean_data(tr); te=clean_data(te,reference_df=tr); clean=pd.concat([tr,te]).sort_index(); clean.to_csv(ROOT/"data/patient_records_clean.csv",index=False); Xtr,Xte,sc,enc=prepare_features(tr,te);ytr,yte=tr[TARGET_COL],te[TARGET_COL]; models={"logistic_regression":LogisticRegression(max_iter=1000,random_state=42),"random_forest":RandomForestClassifier(n_estimators=200,random_state=42)}; res={}
